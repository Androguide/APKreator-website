 ...
 (function($){
    ...
    $.fn.repo = function( options ){
        ...
        calculateHeight = function(el){
                ...
                // clientHeight is element containing fetched results, plus the h5 tag, plus
                // the div repo margin has of 15 pixels.  
                return (el[0].clientHeight + _this.container.find('h5').outerHeight(true) + 15);
            },
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                _this.container.on('click', 'a', function(e){
                        ...
                        if(el.legnth > 0){
                            ...
                            $.ajax({
                                ...
                                success: function(response){
                                 ...
                                 if (localStorage.getItem("tabsArray") != null) {
                                    tabsArray = localStorage.getItem("tabsArray");
                                    var areOpen = tabsArray.length
                                } else {
                                    var areOpen = 1;
                                }


                                tabsArray.push(response.data.name);
                                console.log(tabsArray);
                                localStorage.setItem("tabsArray", tabsArray);

                                for (var i = 0; i < tabsArray.length; i++) {
                                    if (i > areOpen)
                                        $("ourFileTabs").append("<li><a href=\'#tab"+i+"\' data-toggle=\'tab\' >" + tabsArray[i] + "</a></li>");
                                }

                                var currFile = response.data.name
                                var editor = ace.edit("editor");
                                var mode = currFile.split('.');
                                var i = mode.length - 1;
                                if (mode[i] != null && mode[i] != "") {
                                    ...
                                    if(mode[i] == "xml") {
                                        ...
                                        localStorage.setItem('currMode', 'ace/mode/xml');
                                    } else if (mode[i] == "java") {
                                        editor.getSession().setMode("ace/mode/java");
                                        localStorage.setItem('currMode', 'ace/mode/java');
                                    } else if (mode[i] == "js") {
                                        editor.getSession().setMode("ace/mode/javascript");
                                        localStorage.setItem('currMode', 'ace/mode/javascript');
                                    } else if (mode[i] == "html") {
                                        editor.getSession().setMode("ace/mode/html");
                                        localStorage.setItem('currMode', 'ace/mode/html');
                                    } else if (mode[i] == "css") {
                                        editor.getSession().setMode("ace/mode/css");
                                        localStorage.setItem('currMode', 'ace/mode/css');
                                    } else if (mode[i] == "txt") {
                                        editor.getSession().setMode("ace/mode/none");
                                        localStorage.setItem('currMode', 'ace/mode/none');
                                    } else if (mode[i] == "php") {
                                        editor.getSession().setMode("ace/mode/php");
                                        localStorage.setItem('currMode', 'ace/mode/php');
                                    } else if (mode[i] == "rb") {
                                        editor.getSession().setMode("ace/mode/ruby");
                                        localStorage.setItem('currMode', 'ace/mode/ruby');
                                    }

                                }

                                editor.setValue(String(decode64(response.data.content)));
                                editor.clearSelection();
                                $('#file-tab').empty().html(currFile);
                                localStorage.setItem('currFile', currFile);
                                var driveStart = '<div class=\"g-savetodrive\" data-src=\"Main.java\" data-filename=\"';
                                var driveEnd = '\"data-sitename=\"APKreator.com\"></div>';
                            },
                    ...
                    _this.container.find('h5 a').last().remove();
                    ...
                    if(link[0] !== _this.container.find('h5 a')[0])
                        link.addClass('active');
                    _this.container.find('h5 a').slice((link.index()+1),_this.container.find('h5 a').length).remove();
                    transition(el, 'right');
                }
            });
},
error : function(response){
    if(console && console.log)
        ...
        return this.each(function(){
            _this.container = $('<div class="repo"><h5><a href="#" data-id="' + _this.namespace + '_split_default">' + _this.settings.name + '</a></h5><div class="loader"></div><div class="page" id="' + _this.namespace + '_split_default"><ul><li class="titles"><span>name</span></li></ul></div></div>').appendTo($(this));
        });
    };
